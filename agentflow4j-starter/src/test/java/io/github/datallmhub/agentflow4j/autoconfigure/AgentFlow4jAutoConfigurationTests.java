...
class AgentFlow4jAutoConfigurationTests {

    private final ApplicationContextRunner runner = new ApplicationContextRunner()
            .withConfiguration(AutoConfigurations.of(AgentFlow4jAutoConfiguration.class));
    ...
    void disabledWhenPropertyFalse() {
        runner.withPropertyValues("agentflow4j.enabled=false")
    ...
    void micrometerListenerDisabledByProperty() {
        runner.withUserConfiguration(RegistryConfig.class)
                .withPropertyValues("agentflow4j.observability.metrics=false")
    ...
    void llmDrivenRoutingUsesChatClientWhenAvailable() {
        runner.withUserConfiguration(ChatClientConfig.class)
                .withPropertyValues("agentflow4j.squad.default-routing-strategy=LLM_DRIVEN")
                .run(ctx -> assertThat(ctx).hasSingleBean(RoutingStrategy.class));
    }

    @Test
    void llmDrivenFallsBackToFirstWhenNoChatClient() {
        runner.withPropertyValues("agentflow4j.squad.default-routing-strategy=LLM_DRIVEN")
                .run(ctx -> assertThat(ctx).hasSingleBean(RoutingStrategy.class));
    }

    @Test
    void propertiesGettersAndSettersCoverAllFields() {
        AgentFlow4jProperties props = new AgentFlow4jProperties();
        ...
        props.getSquad().setDefaultRoutingStrategy(
                AgentFlow4jProperties.Squad.DefaultRoutingStrategy.LLM_DRIVEN);
        ...
        assertThat(props.getSquad().getDefaultRoutingStrategy())
                .isEqualTo(AgentFlow4jProperties.Squad.DefaultRoutingStrategy.LLM_DRIVEN);
    }
